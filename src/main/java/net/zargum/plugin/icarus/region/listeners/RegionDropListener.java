...
public class RegionDropListener implements Listener {

    private final RegionManager manager;

    public RegionDropListener(RegionManager manager) {
    ...
    public void onDrop(PlayerDropItemEvent event) {
        ...
        if (manager.hasLogsEnabled(player)) {
            ...
            player.sendMessage(Messages.REGION_LOG.toString("drop", manager.getRegionsFormatted(regionsFromFrameLocation)));
        }
        if (manager.getFinalFlagState(player.getLocation(), Flag.DROP) == FlagState.DENY) {
